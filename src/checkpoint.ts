...
// preprod keeps the original filename; other networks get their own file so
// checkpoints from different chains never collide.
const CHECKPOINT_PATH = join(
  process.cwd(),
  NETWORK_NAME === 'preprod' ? 'wallet-checkpoint.json' : `wallet-checkpoint-${NETWORK_NAME}.json`,
);
